...
alfia.sendMessage(anu.id, { image: { url: ppuser }, fileLength: jumhal, mentions: [num], caption: txt3, buttons: buttons, footer: ownername})
} else if (anu.action == 'demote') {
let txt4 = `@${num.split('@')[0]} Demote From ${metadata.subject}`
alfia.sendMessage(anu.id, { image: { url: ppuser },fileLength: jumhal,  mentions: [num], caption: txt4, buttons: buttons, footer: ownername})
}
}
} catch (err) {
console.log(err)
